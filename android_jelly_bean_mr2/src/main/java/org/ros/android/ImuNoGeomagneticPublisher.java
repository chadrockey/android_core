 ...
 */
public class ImuNoGeomagneticPublisher implements NodeMain
{
    ...
    private class ImuThread extends Thread {
        ...
        private final Sensor quaternionSensor;

        private ImuThread(SensorManager sensorManager, SensorListener sensorListener) {
            this.sensorManager = sensorManager;
            this.sensorListener = sensorListener;
            this.accelerationSensor = this.sensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
            this.gyroSensor = this.sensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE);
            this.quaternionSensor = this.sensorManager.getDefaultSensor(Sensor.TYPE_GAME_ROTATION_VECTOR);
        }
        ...
        public void run() {
            ...
            this.sensorManager.registerListener(this.sensorListener, this.accelerationSensor, sensorDelay);
            this.sensorManager.registerListener(this.sensorListener, this.gyroSensor, sensorDelay);
            this.sensorManager.registerListener(this.sensorListener, this.quaternionSensor, sensorDelay);
            Looper.loop();
        }
    ...
    private class SensorListener implements SensorEventListener {
        ...
        private SensorEvent accelerationEvent = null;
        private SensorEvent gyroEvent = null;
        private SensorEvent orientationEvent = null;

        private SensorListener(Publisher<Imu> publisher, boolean hasAcceleration, boolean hasGyro, boolean hasQuaternion) {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if((this.accelerationEvent != null || !this.hasAcceleration) &&
                    (this.gyroEvent != null || !this.hasGyro) &&
                    (this.orientationEvent != null || !this.hasQuaternion)) {
                ...
                if(accelerationEvent != null) {
                    la.setX(accelerationEvent.values[0]);
                    la.setY(accelerationEvent.values[1]);
                    la.setZ(accelerationEvent.values[2]);
    ...
    public ImuNoGeomagneticPublisher(SensorManager manager) {
        this(manager, SensorManager.SENSOR_DELAY_GAME);
    }

    public ImuNoGeomagneticPublisher(SensorManager manager, int sensorDelay) {
        this.sensorManager = manager;
        this.sensorDelay = sensorDelay;
    }

    public GraphName getDefaultNodeName() {
        return GraphName.of("android_/imu_no_geomagnetic_publisher");
    }
    ...
    public void onStart(ConnectedNode node) {
        try {
            this.publisher = node.newPublisher("android/imu_no_geomagnetic", "sensor_msgs/Imu"); //TODO Allow config
            // Determine if we have the various needed sensors
            boolean hasAcceleration = false;
            boolean hasGyro = false;
            boolean hasQuaternion = false;

            List<Sensor> accelerationList = this.sensorManager.getSensorList(Sensor.TYPE_ACCELEROMETER);
            ...
            List<Sensor> quaternionList = this.sensorManager.getSensorList(Sensor.TYPE_GAME_ROTATION_VECTOR);
            if(quaternionList.size() > 0) {
                hasQuaternion = true;
            }

            this.sensorListener = new SensorListener(publisher, hasAcceleration, hasGyro, hasQuaternion);
